...
module.exports = ({
    ...
    var draw = i => {
        ...
        var size = fit( [], vec2.fromValues( file.w, file.h ), clipSize, 'contain' );
        vec2.scale( size, size, scale );
        
        var p = lazyMouse( 1 - i / count );
        
        var offset = vec2.subtract( [], p, vec2.scale( [], size, .5 ) );
        
        // var min = [ 0, 0 ];
        // var max = vec2.subtract( [], clipSize, size )
        
        // offset = vec2.max( offset, vec2.min( [], offset, max ), min );
